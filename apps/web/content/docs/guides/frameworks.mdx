...
`@emoteer/svelte` ships the same component set as React, built for Svelte 5 (runes). The surface is identical — compound primitives (`<EmoteList.Root>`, `<ReactionButton.Popover>`, …), typed props, and the same `data-part` styled defaults. See the per-recipe Svelte code in [Recipes](/docs/recipes/chat-composer) via the React/Svelte toggle.
